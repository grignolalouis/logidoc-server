...
// Server wraps the MCP server (stdio or streamable HTTP).
type Server struct {
...
func NewServer(cfg config.MCPConfig, docSvc port.DocumentService, retSvc port.RetrievalService, logger *slog.Logger) *Server {
	...
	if cfg.Stdio {
		...
		s.stdio.RegisterTool(defs[1], h.getTOC)
		s.stdio.RegisterTool(defs[2], h.getSections)
	} else {
		s.http = mcplib.NewServer("logidoc", "1.0.0",
			mcplib.WithServerAddress(cfg.Addr),
		)
		s.http.RegisterTool(defs[0], h.listDocuments)
		s.http.RegisterTool(defs[1], h.getTOC)
		s.http.RegisterTool(defs[2], h.getSections)
	}
	return s
}
